...
for line in pcontent:
    ...
    for wrd in sentence_words:
        wrd = wrd.strip('.,!?:;()_-')
        wrd = wrd.lower()
        for newobj4 in military_word_count:
            if exact_match(wrd, newobj4.word) != None:
                newobj4.increment_count()
...
print " Total Military word count is {0}".format(military_total_count)
